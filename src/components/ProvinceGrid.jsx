...
import { PROVINCES, ISLANDS } from '@/data/explore-data';
...
export default function ProvinceGrid() {
  ...
  const filtered = PROVINCES
    ...
    .sort((a, b) => sort === 'alpha' ? a.name.localeCompare(b.name) : b.popular - a.popular);
  return (
    <section style={ex.section} id="provinces">
      <div style={ex.secHeader}>
        ...
        <div style={ex.provSortRow}>
          <button onClick={() => setSort('alpha')} style={{ ...ex.sortToggle, ...(sort === 'alpha' ? ex.sortToggleActive : {}) }}>Alfabetis</button>
          <button onClick={() => setSort('popular')} style={{ ...ex.sortToggle, ...(sort === 'popular' ? ex.sortToggleActive : {}) }}>Terpopuler</button>
        </div>
      </div>
      <div style={ex.provFilters}>
        <div style={ex.provSearchWrap}>
          ...
          <input style={ex.provSearch} placeholder="Cari provinsi..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <div style={ex.islandChips}>
          {['Semua', ...ISLANDS.map(i => i.name)].map((i) => (
            <button key={i} onClick={() => setIsland(i)} style={{ ...ex.islandChip, ...(island === i ? ex.islandChipActive : {}) }}>{i}</button>
          ))}
        </div>
      </div>
          ...
          <a key={p.name} href="/" style={ex.provCard}>
            ...
            <div style={ex.provBody}>
              ...
              <div style={ex.provStats}>
                <span><strong>{p.dest}</strong> destinasi</span>
                <span style={ex.provDot}>·</span>
                <span><strong>{p.attr}</strong> atraksi</span>
                <span style={ex.provDot}>·</span>
                <span><strong>{p.desa}</strong> desa</span>
              </div>
            </div>
          </a>
        ))}
      ...
      {filtered.length === 0 && (
        <div style={ex.emptyState}>
          ...
          <button onClick={() => { setSearch(''); setIsland('Semua'); }} style={ex.emptyBtn}>Reset filter</button>
        </div>
      )}
    </section>
  );
